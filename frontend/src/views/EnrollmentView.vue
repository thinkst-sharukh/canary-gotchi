<template>
  <Layout>
    <form @submit.prevent="verifyAuth">
      <div class="flex gap-x-3 p-8 bg-white border rounded-lg shadow-lg shadow-gray-100">
        <div class="w-full space-y-4">
          <Input v-model="name" placeholder="Enter gotchi name" required min="3" max="32" :disabled="loading" />
      ...
      <div class="mt-4 flex flex-col items-center justify-center">
        <Button type="submit" variant="success" :disabled="loading || (!name || !domainHash || !authToken)" class="w-[50%] text-lg">
...
import { existsGotchi, verifyAuthKey } from '@/services/gotchi';
...
onMounted(() => {
  ...
  if (exist === 'true') {
    verifyExistingGotchi()
  }
})

const verifyExistingGotchi = async () => {
  if (loading.value) {
    return;
  }
  ...
  try {
    toast.remove()
    loadingToastId = toast.loading("Verifying existing gotchi...")
    loading.value = true
    const response = await existsGotchi(id as string)
    ...
    if (isAxiosError(error)) {
      ...
      toast.error("Failed to verify existing gotchi")
    }
  } finally {
    loading.value = false
